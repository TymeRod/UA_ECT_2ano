...
class stack {
  ...
  public:
    ...
    void push(T v){ // put a thing in the stack
      if(d_size >= max_size)
    ...
    void put(T v){ // same as push
      push(v);
    }

    T pop(void){ // get and remove a thing from the too of the stack (last in, first out!)
      T v;
    ...
    T get(void){ // same as pop
      return pop();
    }

    T top(void){ // get a thing from the top of the stack
      T v;
...
int main(void){
  ...
  cout << "pop: " << s.pop() << endl; // prints error "pop error: the stack is empty"
  return 0;
}
